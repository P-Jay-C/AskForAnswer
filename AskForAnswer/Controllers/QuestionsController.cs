...
namespace AskForAnswer.Controllers
{
    public class QuestionsController : Controller
    {
		...
		public async Task<IActionResult> Details(int? id)
		{
			if (id == null)
			{
				_logger.LogInformation("Details view requested with a null ID.");
				return NotFound();
			}

			var question = await _context.Questions
				.Include(q => q.Answers)
				.ThenInclude(a => a.User) // Assuming each answer has a User associated with it
				.Include(q => q.User)
			...
			if (question == null)
			{
				_logger.LogInformation($"No question found with ID {id}.");
				return NotFound();
			}
